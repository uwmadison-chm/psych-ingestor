...
The words PI uses, and what each one means here. Where a term is used loosely elsewhere in
the docs, this page wins.

PI borrows its vocabulary from BIDS, because most people reading this have already met it
...
A single PI deployment runs many tasks, for many projects, across a lab.
...
`?participant_id=10351&session=baseline`. It is not something PI creates or tracks a
...
finish. It's identified by a **run ID** that PI generates and returns — not by the link
parameters, though the parameters are what determine which run is meant.

A run is the only thing in PI with a lifecycle. Everything else is either configuration or
stored data.

## Run ID

The identifier PI generates when a run starts, and the only thing a task needs in order to
...
example, `["participant_id", "session"]` means PI tracks participants and sessions
...
always produces a new run with a new number, so PI never has to overwrite or merge
anything.
...
far more than any real participant will produce; PI refuses rather than rolling over.

A task cannot currently refuse a repeat run. Every start gets a run, and PI keeps them all.
A `max_runs` setting is planned; see [configuration.md](configuration.md).

## Run status

Every run is in exactly one of four states. This is the only status vocabulary PI uses —
...
| `finalizing` | The task said it was done. PI has the events and is filing them. | No |
| `complete` | PI is finished with the run. The dataset is whole and where it belongs. | No |
| `abandoned` | The run was never finalized, and PI has waited long enough to give up. | No |
...
never came back. A run that got stuck in `finalizing` is not abandoned; it's a run PI still
...
unit PI stores and counts, and it doesn't interpret their contents.

This is the BIDS sense of the word, not the REDCap one. A REDCap event is closer to what PI
calls a session.
...
PI stores each event ID exactly once, which is what makes it safe to resend events after a
failed request. This is the mechanism the reliability guarantee rests on.

If the same ID arrives twice with the same content, that's a retry and PI ignores it. If it
arrives twice with *different* content, that's a task with a broken counter, and PI refuses
...
The directory PI stores data under. Every task's storage path is relative to it, and a path
...
## What PI doesn't have a word for

### Study

PI has no concept of a study. Tasks are the top level: they're what configuration defines,
what URLs address, and what data directories are named for. A lab running one project with
six tasks has six independent task definitions, and nothing in PI ties them together.
...
nothing PI does today needs it. Tasks that belong to the same project can say so in their
