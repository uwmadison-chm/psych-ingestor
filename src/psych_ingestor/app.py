...
def create_app(config_path: Path) -> FastAPI:
    ...
    The file is read once, here. Restarting the service is how a definition change takes
    effect, which keeps "when does it re-read the file" a question with one answer.
    """
...
class _pig:
    """One database connection for one request, closed when the request is done."""

    def __init__(self, config: Config):
        self.config = config

    def __enter__(self) -> Pig:
        self.connection = db.connect(self.config.database)
        return Pig(self.config, self.connection)
